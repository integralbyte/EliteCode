...
const problems = [
  ...
  makeProblem(140, "plus-one", "Plus One", "Easy", ["Array", "Math"], "plusOne",
    ...
    (seed) => { const digits = Array.from({ length: 1 + (mix(seed, 20) % 24) }, (_, i) => seed % 9 === 0 ? 9 : mix(seed, i + 21) % 10); return caseFrom({ digits }, plusOne(digits)); }, "The array stores a non-negative integer digit by digit. Return the digits after adding one.", ["Input: digits = [1,2,3]\nOutput: [1,2,4]", "Input: digits = [9,9]\nOutput: [1,0,0]", "Input: digits = [0]\nOutput: [1]"],
  ...
  makeProblem(149, "sum-of-two-integers", "Sum of Two Integers", "Medium", ["Math", "Bit Manipulation"], "getSum",
    ...
    (seed) => { const a = (mix(seed, 220) % 200001) - 100000, b = (mix(seed, 221) % 200001) - 100000; return caseFrom({ a, b }, getSum(a, b)); }, "Return the sum of two integers without using `+` or `-` in the core algorithm.", ["Input: a = 1, b = 2\nOutput: 3", "Input: a = -2, b = 3\nOutput: 1", "Input: a = -5, b = -7\nOutput: -12"],
