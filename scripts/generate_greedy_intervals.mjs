...
const nums = (seed, len = 5 + (seed % 8), min = -8, mod = 20) => Array.from({ length: len }, (_, i) => min + ((seed * 17 + i * 7) % mod));
const pos = (seed, len = 5 + (seed % 8), mod = 8) => Array.from({ length: len }, (_, i) => 1 + ((seed * 11 + i * 5) % mod));
...
function intervalSeed(seed) { const count = 3 + (seed % 7); const arr = []; for (let i = 0; i < count; i += 1) { const s = (seed * 5 + i * 3) % 18; arr.push([s, s + 1 + ((seed + i) % 6)]); } return arr; }
...
const problems = [
  ...
  makeProblem(128, "partition-labels", "Partition Labels", "Medium", ["Hash Table", "Two Pointers", "String", "Greedy"], "partitionLabels",
    ...
    (seed) => { const samples = ["ababcbacadefegdehijhklij", "eccbbbbdec", "abac", "caedbdedda"]; const s = samples[seed % samples.length]; return caseFrom({ s }, partitionLabels(s)); }, "Split a string into as many parts as possible so each letter appears in at most one part.", ["Input: s = ababcbacadefegdehijhklij\nOutput: [9,7,8]", "Input: s = eccbbbbdec\nOutput: [10]", "Input: s = abc\nOutput: [1,1,1]"],
  ...
  makeProblem(129, "valid-parenthesis-string", "Valid Parenthesis String", "Medium", ["String", "Dynamic Programming", "Stack", "Greedy"], "checkValidString",
    ...
    (seed) => { const chars = ["(", ")", "*"]; const s = Array.from({ length: 1 + (seed % 12) }, (_, i) => chars[(seed + i * 2) % 3]).join(""); return caseFrom({ s }, validParenStar(s)); }, "Return whether `*` can be treated as `(`, `)`, or empty to make the parenthesis string valid.", ["Input: s = (*)\nOutput: true", "Input: s = (*))\nOutput: true", "Input: s = )*(\nOutput: false"],
